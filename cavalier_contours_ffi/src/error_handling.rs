...
pub struct LastErrorData {
    ...
    /// Data associated with the last error that occurred, may have information about the function called and inputs received.
...
///
/// `error_msg` and `error_report` are turned into CStrings from bytes, bytes must not include any nulls.
